...
class APICaller {
    ...
    func getDiscoverMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.baseURL)/3/discover/movie?api_key=\(Constants.API_KEY)&language=en-US") else { return }
    ...
    func getMovie(with query: String, completion: @escaping (Result<VideoElement, Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.youtubeBaseURL)query=\(query)&key=\(Constants.youtubeAPI_KEY)") else { return }
